...
function deepDifMod(preMod, nextMod) {
    ...
    if (description !== next.description) {
        details.push(`${label} 的描述信息更新，不需要更新`);
    }
    const modifiedInterfaceListMap = {};
    ...
    const newMsgs = newInters
        .map(inter => {
        modifiedInterfaceListMap[inter.name] = true;
        return inter.name;
    })
    ...
    updateInters.map(tInter => {
        ...
        if (msgs && msgs.length) {
            modifiedInterfaceListMap[tInter.name] = true;
        }
    });
    return {
        msgs: [...details, ...updateMsgs, ...delMsgs, ...newMsgs],
        modifiedInterfaces: Object.keys(modifiedInterfaceListMap)
    };
}
...
function diff(preModels, nextModels, isMod = true) {
    ...
    const delModels = deletedEntities.map(model => (Object.assign({}, model, { details: [
            `删除${label} ${model.name}，请使用update${isMod ? "Mod" : "Bo"}命令更新`
        ] })));
    const newModels = newEntities.map(model => (Object.assign({}, model, { details: [
            `新增${label} ${model.name}，请使用update${isMod ? "Mod" : "Bo"}命令更新`
        ] })));
    const updateModels = [];
    modifiedEntities.forEach(tEntity => {
        ...
        if (isMod) {
            const { msgs, modifiedInterfaces } = deepDifMod(preEntity, nextEntity);
            if (msgs.length) {
                updateModels.push(Object.assign({}, nextEntity, { details: msgs, modifiedInterfaceList: modifiedInterfaces }));
            }
        }
        else {
