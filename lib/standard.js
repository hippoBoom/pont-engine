...
class StandardDataType extends Contextable {
    ...
    getDefName(originName) {
        ...
        if (this.isDefsType) {
            name = originName
                ? `defs.${utils_1.pascalCase(originName)}.${utils_1.pascalCase(this.typeName)}`
                : `defs.${utils_1.pascalCase(this.typeName)}`;
        }
        return name;
    }
    getEnumType() {
        ...
        try {
            ...
            const baseClasses = localDataObject.baseClasses.map(base => {
                const props = base.properties.map(prop => {
                    return new Property(Object.assign({}, prop, { dataType: StandardDataType.constructorFromJSON(prop.dataType, originName, defNames) }));
                });
