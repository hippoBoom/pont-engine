...
import { format, array2Obj } from "../utils";
...
export class FileStructures {
  ...
  getMultipleOriginsFileStructures() {
    ...
    return {
      ...
      "api-lock.yml": this.getLockContent.bind(this)
    };
  }
  ...
  getOriginFileStructures(
    ...
    dataSource.mods.forEach(mod => {
      ...
      mod.interfaces.forEach(inter => {
        ...
        // currMod["index.d.ts"] = generator.getModDeclaration.bind(
        //   generator,
        //   mod
        // );
        currMod[`${inter.name}.d.ts`] = generator.getInterfaceDeclaration.bind(
          generator,
          inter,
          mod
        );
        currMod[
          `${inter.name}.lock.yml`
        ] = generator.getObjectLockFileContent.bind(generator, inter);
      });
    ...
    dataSource.baseClasses.forEach(baseClass => {
      baseClasses[
        `${baseClass.name}.d.ts`
      ] = generator.getBaseClassInDeclaration.bind(generator, baseClass);
      baseClasses[
        `${baseClass.name}.lock.yml`
      ] = generator.getObjectLockFileContent.bind(generator, baseClass);
    });
  ...
  getLockContent() {
    const pureDataSources = JSON.parse(
      JSON.stringify(this.generators.map(ge => ge.dataSource))
    );

    const dataSource = pureDataSources.map((source: StandardDataSource) => ({
      name: source.name,
...
export class CodeGenerator {
  ...
  getInterfaceDeclaration(inter: Interface, mod: Mod) {
    return `
      /**
       * ${inter.description}
       */
  ...
  /** 获取接口实现内容的代码 */
  getInterfaceContent(inter: Interface, _mod?: Mod) {
    const bodyParams = inter.getBodyParamsCode();
  ...
  getObjectLockFileContent(obj: any) {
    const objLock = JSON.parse(JSON.stringify(obj));
    if (objLock.properties) {
      objLock.properties = array2Obj(objLock.properties);
    }
    if (objLock.parameters) {
      objLock.parameters = array2Obj(objLock.parameters);
    }
...
export class FilesManager {
  ...
  constructor(
    private fileStructures: FileStructures,
