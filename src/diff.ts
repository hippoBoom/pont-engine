...
export interface Model extends BaseClass {
  name: string;
  details?: string[];
  modifiedInterfaceList?: string[];
  modifiedBoList?: string[];
}
...
function deepDifMod(preMod: Mod, nextMod: Mod): { [key: string]: string[] } {
  ...
  const modifiedInterfaceListMap: { [key: string]: any } = {};
...
export function diff(
  ...
  modifiedEntities.forEach(tEntity => {
    ...
    if (isMod) {
      const { msgs, modifiedInterfaces } = deepDifMod(
        preEntity as Mod,
        nextEntity as Mod
      );
      ...
      if (msgs.length) {
        updateModels.push({
          ...nextEntity,
          details: msgs,
          modifiedInterfaceList: modifiedInterfaces
        } as Model);
      }
    } else {
