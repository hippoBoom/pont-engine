...
import { getDuplicateById, pascalCase } from "./utils";
...
export class StandardDataType extends Contextable {
  ...
  static constructorFromJSON(
    ...
    if (typeArgs.length) {
      const instance: StandardDataType = new StandardDataType(
        typeArgs.map(arg =>
          StandardDataType.constructorFromJSON(arg, originName, defNames)
        ),
        typeName,
        isDefsType,
        templateIndex
  ...
  getDefName(originName) {
    ...
    if (this.isDefsType) {
      name = originName
        ? `defs.${pascalCase(originName)}.${pascalCase(this.typeName)}`
        : `defs.${pascalCase(this.typeName)}`;
    }
